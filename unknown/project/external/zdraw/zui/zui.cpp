...
namespace
{
	...
	struct input_state
	{
		...
		bool x1_mouse_down{ false };
		bool x1_mouse_clicked{ false };
		bool x1_mouse_released{ false };

		bool x2_mouse_down{ false };
		bool x2_mouse_clicked{ false };
		bool x2_mouse_released{ false };
		...
		void clear_one_offs( )
		{
			...
			this->right_mouse_released = false;
			this->x1_mouse_clicked = false;
			this->x1_mouse_released = false;
			this->x2_mouse_clicked = false;
			this->x2_mouse_released = false;
	...
	public:
		...
		void update( ) noexcept
		{
			...
			// Sync one-off events and queues
			this->m_current.mouse_clicked = this->m_pending.mouse_clicked;
			this->m_current.mouse_released = this->m_pending.mouse_released;
			this->m_current.right_mouse_clicked = this->m_pending.right_mouse_clicked;
			this->m_current.right_mouse_released = this->m_pending.right_mouse_released;
			this->m_current.x1_mouse_clicked = this->m_pending.x1_mouse_clicked;
			this->m_current.x1_mouse_released = this->m_pending.x1_mouse_released;
			this->m_current.x2_mouse_clicked = this->m_pending.x2_mouse_clicked;
			this->m_current.x2_mouse_released = this->m_pending.x2_mouse_released;
			this->m_current.scroll_delta = this->m_pending.scroll_delta;

			// Swap queues
			this->m_current.char_count = this->m_pending.char_count;
			this->m_current.char_buffer = this->m_pending.char_buffer;
			this->m_current.key_press_count = this->m_pending.key_press_count;
			this->m_current.key_press_buffer = this->m_pending.key_press_buffer;
			this->m_current.key_release_count = this->m_pending.key_release_count;
			this->m_current.key_release_buffer = this->m_pending.key_release_buffer;

			// Clear pending one-offs for next frame
			this->m_pending.clear_one_offs( );
		}

		void clear_frame_events( ) noexcept
		{
			// Reset current frame one-offs
			this->m_current.clear_one_offs( );
		}
		...
		bool process_wndproc_message( UINT msg, WPARAM wparam, LPARAM lparam ) noexcept
		{
			...
			case WM_LBUTTONDOWN:
			{
				...
				this->m_pending.mouse_clicked = true;
				return true;
			}
			...
			case WM_LBUTTONUP:
			{
				...
				this->m_pending.mouse_released = true;
				return true;
			}
			...
			case WM_RBUTTONDOWN:
			{
				...
				this->m_pending.right_mouse_clicked = true;
				return true;
			}
			...
			case WM_RBUTTONUP:
			{
				...
				this->m_pending.right_mouse_released = true;
				return true;
			}
			...
			case WM_XBUTTONDOWN:
			{
				...
				if ( button == XBUTTON1 )
				{
					this->m_current.x1_mouse_down = true;
					this->m_pending.x1_mouse_clicked = true;
				}
				else if ( button == XBUTTON2 )
				{
					this->m_current.x2_mouse_down = true;
					this->m_pending.x2_mouse_clicked = true;
				}
				return true;
			}
			...
			case WM_XBUTTONUP:
			{
				...
				if ( button == XBUTTON1 )
				{
					this->m_current.x1_mouse_down = false;
					this->m_pending.x1_mouse_released = true;
				}
				else if ( button == XBUTTON2 )
				{
					this->m_current.x2_mouse_down = false;
					this->m_pending.x2_mouse_released = true;
				}
				return true;
			}
			...
			case WM_MOUSEWHEEL:
			{
				...
				this->m_pending.scroll_delta += delta;
				return true;
			}
			...
			case WM_SYSKEYDOWN:
			{
				...
				{
					this->m_pending.push_key_press( vk );
			...
			{
				const auto vk = static_cast< int >( wparam );
				this->m_pending.push_key_release( vk );
		...
		[[nodiscard]] bool x1_mouse_down( ) const noexcept { return this->m_current.x1_mouse_down; }
		[[nodiscard]] bool x1_mouse_clicked( ) const noexcept { return this->m_current.x1_mouse_clicked; }
		[[nodiscard]] bool x1_mouse_released( ) const noexcept { return this->m_current.x1_mouse_released; }

		[[nodiscard]] bool x2_mouse_down( ) const noexcept { return this->m_current.x2_mouse_down; }
		[[nodiscard]] bool x2_mouse_clicked( ) const noexcept { return this->m_current.x2_mouse_clicked; }
		[[nodiscard]] bool x2_mouse_released( ) const noexcept { return this->m_current.x2_mouse_released; }
	...
	public:
		combo_overlay( widget_id id, const zui::rect& anchor, float width, const std::vector<std::string>& items, int* current_item, std::function<void( )> on_change = nullptr ) : overlay{ id, anchor }, m_width{ width }, m_items{ items }, m_current_item{ current_item }, m_on_change{ std::move( on_change ) }
		{
			...
			if ( current_item && *current_item >= 0 && *current_item < static_cast< int >( items.size( ) ) )
			{
				...
				this->m_selected_item = *current_item;
			}
		}
		...
		void set_selected_item( int val ) { this->m_selected_item = val; }
...
namespace zui {
	...
	void end( )
	{
		notifications::g_notifications.draw( );
		ctx( ).end_frame( );
	}
	...
	bool combo( std::string_view label, int& current_item, const char* const items[ ], int items_count, float width )
	{
		...
		{
			current_item = popup->get_selected_item( );
