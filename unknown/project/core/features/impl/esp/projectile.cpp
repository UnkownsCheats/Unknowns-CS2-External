        ...
        for ( const auto& proj : systems::g_collector.projectiles( ) )
        {
            ...
            if ( proj.subtype == systems::collector::projectile_subtype::molotov_fire )
            {
                ...
                if ( proj.expire_time > 0.0f )
                {
                    ...
                    const auto remaining = std::max<float>( 0.0f, proj.expire_time - current_time );
                    const auto frac = std::clamp<float>( remaining / inferno_duration, 0.0f, 1.0f );
            ...
            if ( proj.subtype == systems::collector::projectile_subtype::smoke_grenade && proj.smoke_active )
            {
                ...
                const auto remaining = std::max<float>( 0.0f, smoke_duration - ( current_time - smoke_start ) );
                const auto frac = std::clamp<float>( remaining / smoke_duration, 0.0f, 1.0f );
            ...
            if ( proj.subtype == systems::collector::projectile_subtype::decoy && proj.effect_tick_begin > 0 )
            {
                ...
                const auto remaining = std::max<float>( 0.0f, fuse - ( current_time - throw_time ) );
                const auto frac = std::clamp<float>( remaining / fuse, 0.0f, 1.0f );
