...
namespace systems {
	...
	std::optional<collector::planted_c4> collector::bomb( ) const
	{
		std::shared_lock lock( this->m_mutex );
		return this->m_bomb;
	}
	...
	void collector::collect_players( const std::vector<entities::cached>& raw )
	{
		...
		for ( const auto& entry : raw )
		{
			...
			p.ranking = g::memory.read<std::int32_t>( entry.ptr + SCHEMA( "CCSPlayerController", "m_iCompetitiveRanking"_hash ) );
	...
	void collector::collect_bomb( const std::vector<entities::cached>& raw )
	{
		std::optional<planted_c4> fresh{};

		for ( const auto& entry : raw )
		{
			if ( entry.type != entities::type::bomb )
			{
				continue;
			}
			...
			planted_c4 b{};
			b.ptr = entry.ptr;
			b.is_ticking = g::memory.read<bool>( entry.ptr + SCHEMA( "C_PlantedC4", "m_bBombTicking"_hash ) );
			...
			b.blow_time = g::memory.read<float>( entry.ptr + SCHEMA( "C_PlantedC4", "m_flC4Blow"_hash ) );

			const auto game_scene_node = g::memory.read<std::uintptr_t>( entry.ptr + SCHEMA( "C_BaseEntity", "m_pGameSceneNode"_hash ) );
			if ( game_scene_node )
			{
				b.origin = g::memory.read<math::vector3>( game_scene_node + SCHEMA( "CGameSceneNode", "m_vecAbsOrigin"_hash ) );
			}

			b.defuser_handle = g::memory.read<std::uint32_t>( entry.ptr + SCHEMA( "C_PlantedC4", "m_hBombDefuser"_hash ) );
			b.is_being_defused = ( b.defuser_handle != 0 && b.defuser_handle != 0xFFFFFFFF );
			b.defuse_countdown = g::memory.read<float>( entry.ptr + SCHEMA( "C_PlantedC4", "m_flDefuseCountDown"_hash ) );
