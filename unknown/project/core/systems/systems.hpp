...
namespace systems {
	...
	{
	public:
		[[nodiscard]] std::int32_t lookup( const char* class_name, std::uint32_t field_hash, const char* field_expr = nullptr );
	...
	public:
		...
		struct planted_c4
		{
			...
			float blow_time{};
			bool is_ticking{};
			std::uint32_t defuser_handle{};
			float defuse_countdown{};
		...
		[[nodiscard]] std::optional<planted_c4> bomb( ) const;
	...
	private:
		...
		void collect_bomb( const std::vector<entities::cached>& raw );
		...
		std::optional<planted_c4> m_bomb{};
		mutable std::shared_mutex m_mutex{};
	};
...
#define SCHEMA( class_name, field_hash ) \
	[]( ) -> std::int32_t { \
		static const auto val = systems::g_schemas.lookup( class_name, field_hash, #field_hash ); \
		return val; \
	}( )
